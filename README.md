...
For scripts, `--check --json` prints the same findings as a single JSON array on stdout — one
`{file, line, tag}` entry per stale block — so CI can consume the result without parsing text.
Nothing stale prints an empty array. Failed directives are not part of the array — they surface on
stderr and keep the exit code non-zero — so stdout is always valid JSON:
...
[
  {
    "file": "README.md",
    "line": 23,
    "tag": "<!-- cmd: echo This line is regenerated by commentsh on every run. -->"
  }
]
```
